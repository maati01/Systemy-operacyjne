...
void dynamic_handler(){
    ...
    int(*reserve_memory)(int) = (void (*) (int))dlsym(handle, "reserve_memory");
    if(dlerror() != NULL){
        printf("ERROR\n");
    }

    void(*count_lines_and_chars)(int size, char** files) = (void (*)(int size, char** files))dlsym(handle, "count_lines_and_chars");
    if(dlerror() != NULL){
        printf("ERROR\n");
    }
