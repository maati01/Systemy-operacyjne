...
void dynamic_handler(){
    ...
    void(*create_table_d)();
    create_table_d = dlsym(handle, "create_table");
    if(dlerror() != NULL){
        printf("ERROR\n");
    }

    void(*remove_block_d)();
    remove_block_d = dlsym(handle, "remove_block");
    if(dlerror() != NULL){
        printf("ERROR\n");
    }

    void(*reserve_memory_d)();
    reserve_memory_d = dlsym(handle, "reserve_memory");
    if(dlerror() != NULL){
        printf("ERROR\n");
    }

    void(*count_lines_and_chars_d)();
    count_lines_and_chars_d = dlsym(handle, "count_lines_and_chars");
    if(dlerror() != NULL){
        printf("ERROR\n");
    }
